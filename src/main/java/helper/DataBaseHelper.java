...
public class DataBaseHelper {
    ...
    private static int executeUpdate(String sql, Object... params) {
        ...
        try {
            ...
            //保存数据库操作日志
            SyncService.saveSqlLog(SyncService.generateSqlLog(sql, params));
        } catch (SQLException e) {
     ...
     * @return 受影响行数
     */
    private static int executeUpdateForLog(String sql, Object... params) {
        int rows = 0;
        try {
            Connection conn = getConnection();
            rows = QUERY_RUNNER.update(conn, sql, params);
        } catch (SQLException e) {
            logger.error("execute log operation failure", e);
        }
        return rows;
    }
     ...
     * @return
     */
    public static <T> boolean insertEntityForLog(String tableName, Map<String, Object> fieldMap) {
        if (fieldMap.isEmpty()) {
            logger.error("Can Not insert entity:fieldMap is empty.");
            return false;
        }
        ...
        sql += columns + " VALUES " + values;
        Object[] params = fieldMap.values().toArray();
        return executeUpdateForLog(sql, params) == 1;
    }
     ...
     * @return
     */
    public static <T> boolean updateEntityForLog(String tableName, long id, Map<String, Object> fieldMap) {
        if (fieldMap.isEmpty()) {
